...
public:
  ...
  enum std_t { STD_DEFAULT=0,
               STD_NTSC=V4L2_STD_NTSC_M, 
               STD_PAL=V4L2_STD_PAL_B, 
               STD_SECOM=V4L2_STD_SECAM_B } std;
  Cam(const char *device, mode_t _mode = MODE_RGB,
      int _width=640, int _height=480, int _fps=30, unsigned _num_buff=2, int _std=STD_DEFAULT);
  ~Cam();
