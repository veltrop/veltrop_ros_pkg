...
Cam::Cam(const char *_device, mode_t _mode, int _width, int _height, int _fps,
         unsigned _num_buff, int _std)
: mode(_mode), device(_device),
  motion_threshold_luminance(100), motion_threshold_count(-1),
  width(_width), height(_height), fps(_fps), std_id(_std), 
  NUM_BUFFER(_num_buff), rgb_frame(NULL)
{
  mem = (void**)malloc(sizeof(void*) * NUM_BUFFER);
...
*/
  ...
  if (std_id)
    if (ioctl(fd, VIDIOC_S_STD, &std_id) < 0)
      //throw std::runtime_error("unable to set video standard");
      perror("unable to set video standard");
