...
void Camera::Init() {
  ...
  if(0 == xioctl (fd, VIDIOC_CROPCAP, &cropcap)) {
    ...
    fmt.fmt.pix.pixelformat = V4L2_PIX_FMT_YUYV; //v4l2_fourcc('B','A','8','1'); //V4L2_PIX_FMT_RGB565; //V4L2_PIX_FMT_YUYV;
    fmt.fmt.pix.field       = V4L2_FIELD_NONE;//V4L2_FIELD_SEQ_BT;//V4L2_FIELD_BOTTOM;//V4L2_FIELD_TOP;//V4L2_FIELD_INTERLACED;//V4L2_FIELD_ALTERNATE;//V4L2_FIELD_SEQ_TB;//V4L2_FIELD_ANY;
...
//if(-1==xioctl(fd, VIDIOC_ENUMSTD, &s))
//  errno_exit("VIDIOC_ENUMSTD");

// TODO: WTF?
//struct v4l2_streamparm p;
//if(-1==xioctl(fd, VIDIOC_G_PARM
...
v4l2_std_id stdid = V4L2_STD_NTSC_M;
if(-1 == xioctl (fd, VIDIOC_S_STD, &stdid))
    errno_exit ("VIDIOC_S_STD");
...
unsigned char *Camera::Get(unsigned char *ext_data) {
  struct v4l2_buffer buf;
//  unsigned int i;
...
*/
    ...
    case IO_METHOD_MMAP:
      ...
      if (!ext_data)
        ...
        memcpy(ext_data, (unsigned char *)buffers[buf.index].start, buffers[buf.index].length);
